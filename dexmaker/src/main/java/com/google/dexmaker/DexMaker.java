...
public final class DexMaker {
    ...
    static File spitToJar(File dexCache, byte[] dex) throws IOException {
    ...
    public ClassLoader generateAndLoad(ClassLoader parent, File dexCache) throws IOException {
         ...
         */
        File result = spitToJar(dexCache, dex);
        try {
    ...
    /**
     * Generates a dex file and loads its types into the current process with
     * the given ClassLoader.
     *
     * @param loader   the ClassLoader to be used when loading our generated types
     * @param dexCache the destination directory where generated and optimized
     *                 dex files will be written. If null, this class will try to guess the
     *                 application's private data dir.
     * @see #generateAndLoad(ClassLoader, java.io.File)
     */
    public ClassLoader generateAndLoadWith(final ClassLoader loader, File dexCache) throws IOException {
        if (dexCache == null) {
            dexCache = guessDexCache();
        }
        ...
        final Method loadClass;
        try {
            Class<?> DexFile = Class.forName("dalvik.system.DexFile");
            dexFile = DexFile.getConstructor(File.class).newInstance(result);
            loadClass = DexFile.getMethod("loadClass", String.class, ClassLoader.class);
        } catch (ClassNotFoundException e) {
            throw new UnsupportedOperationException("load() requires a Dalvik VM", e);
        } catch (InvocationTargetException e) {
            throw new RuntimeException(e.getCause());
        ...
        return new ClassLoader(loader) {
            ...
            protected Class<?> findClass(String name) throws ClassNotFoundException {
                Class<?> c;
                try {
                    c = (Class<?>) loadClass.invoke(dexFile, name, loader);
                } catch (Exception e) {
                    throw new ClassNotFoundException("Class couldn't be loaded: " + name, e);
                }
